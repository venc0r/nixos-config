...
let
  scripts = import ../hosts/scripts.nix { inherit pkgs; };
in
{
  programs.tmux = {
    enable = true;
    ...
    extraConfig = ''
      ...
      # Custom script bindings (using display-popup for interactive scripts)
      bind-key i display-popup -E "${scripts.tmux-cht}/bin/tmux-cht"
      bind-key y display-popup -E "${scripts.tmux-cal}/bin/tmux-cal"
      bind-key g display-popup -E "${scripts.tmux-sessionizer}/bin/tmux-sessionizer"
      ...
      bind-key -T copy-mode-vi y send-keys -X copy-pipe-and-cancel '${if pkgs.stdenv.isDarwin then "pbcopy" else "${pkgs.xclip}/bin/xclip -in -selection clipboard"}'
