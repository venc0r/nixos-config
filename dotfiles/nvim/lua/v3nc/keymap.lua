...
vim.keymap.set("v", "<leader>ave",
    [[dmm:r!ansible-vault encrypt_string '<C-R>"' --encrypt-vault-id default --vault-id default@<(rbw get ansible_vault)<CR><CR>'mJ]])
vim.keymap.set("v", "<leader>avd", "10<gv:!ansible-vault decrypt --vault-id=default@<(rbw get ansible_vault)<CR>")
vim.keymap.set("n", "<leader>avf",
    "<CMD>!ansible-vault encrypt --encrypt-vault-id default --vault-password-file <(rbw get ansible_vault) %<CR>")
...
vim.keymap.set(
    "n",
    "<leader>yl",
    ("O# yaml-language-server: $schema=file://" .. os.getenv("HOME") .. "/kubernetes_schema/kubernetes.json<Esc>")
)
...
vim.api.nvim_create_autocmd('FileType', {
  ...
  callback = function(event)
    vim.keymap.set('n', '<leader>md', function()
      vim.fn.jobstart({ 'inlyne', vim.api.nvim_buf_get_name(event.buf) }, { detach = true })
    end, { buffer = event.buf, desc = 'Preview markdown in inlyne' })
  end,
})
